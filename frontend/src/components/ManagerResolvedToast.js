...
export default function ManagerResolvedToast({ data, onClose }) {
  ...
  // Auto-fermeture 18 s si pas d'interaction, 3.5 s après envoi
  useEffect(() => {
    if (sent)        { const t = setTimeout(close, 3500);  return () => clearTimeout(t); }
    if (!showInput)  { const t = setTimeout(close, 18000); return () => clearTimeout(t); }
  }, [sent, showInput]);
  ...
  return (
    <>
      <style>{`
        ...
        .mgr-enter { animation: slideInRight  0.45s cubic-bezier(0.22,1,0.36,1) both; }
        .mgr-exit  { animation: slideOutRight 0.38s ease-in both; }
      `}</style>

      <div
        className={visible ? "mgr-enter" : "mgr-exit"}
        style={{
      ...
      >
        <div style={{ background:"#fff", borderRadius:18, overflow:"hidden",
          ...
          <div style={{ height:4,
            ...
            backgroundSize:"300% auto", animation:"shimmerBar 3s linear infinite" }} />
          ...
          <div style={{ padding:"14px 16px 12px", display:"flex", alignItems:"flex-start", gap:12,
            ...
            <div style={{ width:44, height:44, borderRadius:12, flexShrink:0,
              ...
              fontSize:20, boxShadow:"0 4px 12px rgba(34,197,94,0.3)" }}>✅</div>
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:14, fontWeight:700, color:"#111827", marginBottom:2 }}>
                ...
                {data.type && <span style={{ fontWeight:500, color:"#6b7280", fontSize:13 }}> · {data.type}</span>}
              </div>
            ...
            <button onClick={close} style={{ background:"none", border:"none", cursor:"pointer",
              color:"#9ca3af", fontSize:16, padding:"2px 4px", borderRadius:6,
          ...
          <div style={{ padding:"12px 16px 16px" }}>
            {sent ? (
              <div style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 14px",
                ...
                <div>
                  <div style={{ fontSize:13, fontWeight:700, color:"#15803d" }}>Message envoyé !</div>
                  <div style={{ fontSize:11, color:"#16a34a" }}>Le citoyen a été notifié.</div>
                </div>
              </div>
            ) : showInput ? (
              <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                <textarea
                  ...
                  style={{ width:"100%", padding:"10px 12px", borderRadius:10,
                    border:"1.5px solid #d1d5db", fontSize:12, color:"#111827",
                    fontFamily:"'DM Sans','Roboto',sans-serif",
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <button onClick={() => setShowInput(false)} style={{ flex:1, padding:"8px 0",
                    borderRadius:9, border:"1.5px solid #e5e7eb", background:"#fff",
                    color:"#6b7280", fontSize:12, fontWeight:600, cursor:"pointer",
                    fontFamily:"'DM Sans','Roboto',sans-serif" }}>Annuler</button>
                  <button onClick={handleSendMessage} disabled={sending} style={{ flex:2,
                    padding:"8px 0", borderRadius:9, border:"none",
                    background: sending ? "#d1d5db" : "linear-gradient(135deg,#16a34a,#22c55e)",
                    ...
                    {sending
                      ? <><span style={{ width:12, height:12, border:"2px solid #fff",
                          borderTopColor:"transparent", borderRadius:"50%", display:"inline-block",
                          animation:"spin 0.7s linear infinite" }}/> Envoi…</>
                      : "📨 Envoyer"}
                  </button>
                </div>
              </div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={close} style={{ flex:1, padding:"9px 0", borderRadius:9,
                  border:"1.5px solid #e5e7eb", background:"#fff", color:"#6b7280",
                  fontSize:12, fontWeight:600, cursor:"pointer",
                  fontFamily:"'DM Sans','Roboto',sans-serif", transition:"all 0.15s" }}
                  onMouseEnter={e => e.currentTarget.style.background="#f9fafb"}
                  onMouseLeave={e => e.currentTarget.style.background="#fff"}>Ignorer</button>
                <button onClick={() => setShowInput(true)} style={{ flex:2, padding:"9px 0",
